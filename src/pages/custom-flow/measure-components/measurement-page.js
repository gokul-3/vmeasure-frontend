...
import { BarcodeDataCategory, MeasurementTriggerSrc, MeasurementBackgroundColor, MeasurementState, SyncRetryMode, ProcessingState, MeasurementStateInfoReason, ObjectTpe, Certificates, SmartMeasurementTriggerState, ExternalInputs, TimeoutStateInfoReason } from "../../../constants";
import { cancelBarcodeValidate, clearMeasurementData, getAnnotatedImage, validateBarcode, validateReferenceBoxBarcode, checkRefBoxCondition, discardMeasurementData } from "../../../services/measurement.service";
import appState from "../../../redux/reducers/measurement-states"
import MeasurementTriggerTimer from "../../measurement/components/measurement-trigger-timer";
import MeasurementDiscardTimer from "../../measurement/components/measurement-discard-timer";
import DimensionInfoArea from "../../measurement/components/dimension-info-area";
import WeightInfoArea from "../../measurement/components/weight-info-area";
import { useTranslation } from 'react-i18next';
import VideoStream from "../../../components/video-stream/video-stream";
import VolumetricDivisor from "../../measurement/components/volumentric-divisor";
import BarcodeField from "../../measurement/components/barcode-field";
import MeasurementClearTimer from "../../measurement/components/measurement-clear-timer";
import MeasurementPageCustomField from "../../measurement/components/measurement-page-custom-field";
import AdditionalBarcode from "../../measurement/components/additional-barcodes";
import AdditionalResultData from "../../measurement/components/additional-data";
import MeasurementInfo from "../../measurement/components/measurement-info";
import BarcodeValidationRetry from "../../measurement/components/barcode-validation-retry";
import WhiteAnnotatedImage from '../../../assets/images/white-annotated.jpg'
import MeasurementUploadRetry from "../../measurement/components/measurement-upload-retry";
import QRPopup from "../../measurement/components/qr-popup";
import navigationController from "../../../redux/reducers/nav-bar-controller";
import BottleMode from "../../measurement/components/bottle-mode";
import MeasurementDuration from "../../measurement/components/measurement-duration";
import { useKeyboard } from "../../../hooks/useKeyboard";
import { ObjectTypeIcon } from "../../measurement/components/object-type-icon";
import { NTEPModeIcon } from "../../measurement/components/ntep-icon";
...
function MeasurementPage({ onAction, dynamicPageData = {}, onMeasurementClearStateStarted, onMeasurementProcessCompleted, barcodeValue = null }) {
  ...
  useEffect(() => {
    if (barcodeValue) {
      ...
      barcodeValueProps.current.currentValue = barcodeValue;
  ...
  const { pageAttribute } = useSelector(state => state.customFlow);
  ...
  const handleReferenceBoxScan = async () => {
     ...
     * 2. If auto clear is not enabled, measurement state will be stayed in PROCESS_COMPLETED state
 ...
 */
  const handleMeasurementStateChange = async () => {
    ...
    switch (measurement_state.currentState) {
      ...
      case MeasurementState.MEASUREMENT_UPLOAD:
        //changes made for custom workflow
        if (Boolean(pageAttribute?.pageType?.data?.autoUpload)) {
          if (measurement_state.processingState === ProcessingState.INIT) {
            dispatch(appState.extraActions.saveMeasurementViaCustom({
              is_discard: false,
  ...
  useEffect(() => {
    ...
    /**
     * retain barcode value on-measurement failure incase barcode is sent from custom service
     */
    if (measurement_state?.currentState === MeasurementState.INIT &&
      measurement_state?.processingState === ProcessingState.INIT && 
      barcodeValue
                ...
                <Grid container item xs={12} height={'15%'} spacing={WORK_AREA_SPACING} marginBottom={WORK_AREA_SPACING}>
                  <Grid item xs={validateCetrificateMode([Certificates.NTEP]) ? 5 : 6} display="flex" alignItems="center">
                    ...
                    >
                      ...
                      {pageAttribute?.title || t('measurement_page.title')}
                    </Typography>
                  </Grid>
